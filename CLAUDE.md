...
Detects whether a command is a "native" CLI (claude/codex/opencode/kimi) or a generic "contract" command. Native launchers use CLI-specific flags (e.g., `claude -p --output-format stream-json`). Contract launchers pass `BUDDY_ACTOR`, `BUDDY_MODE` env vars. Session resumption uses `--resume` (Claude), `resume` (Codex), `--session` (OpenCode/Kimi).
